...
class controllerClass {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $anwr = modelClass::newPrograma($_POST['idProg'], $_POST['txtDescripcion'], $_POST['txtEstado']);
      if ($anwr === true) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      if (is_array($certificate)) {
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              $args['idProg'] = $data[0]['cod_prog'];
              $args['txtDescripcion'] = $data[0]['desc_prog'];
      ...
      $data['desc_prog'] = $_POST['txtDescripcion'];
      $data['estado'] = $_POST['txtEstado'];
      $rsp = modelClass::updatePrograma($_GET['id'], $data);
      if ($rsp === true) {
  ...
  public function delete() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      $rsp = modelClass::delPrograma($_GET['id']);
      if ($rsp === true) {
