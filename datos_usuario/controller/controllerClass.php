...
class controllerClass {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $anwr = modelClass::newDatUser($_POST['txtId'], $_POST['txtIdUser'], $_POST['txtNombre'], $_POST['txtApellido'], $_POST['txtTel'], $_POST['txtDireccion'], $_POST['txtLocalidad']);
      if ($anwr === true) {
  ...
  public function update() {
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
        ...
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              ...
              $args['txtIdUser'] = $data[0]['usuario_id'];
      ...
      $rsp = modelClass::updateDatUser($_GET['id'], $data);
      if ($rsp === true) {
  ...
  public function delete() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      $rsp = modelClass::delDatUser($_GET['id']);
      if ($rsp === true) {
