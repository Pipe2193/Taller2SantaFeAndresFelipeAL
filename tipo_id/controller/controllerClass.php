...
class controllerClass {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $anwr = modelClass::newTid($_POST['txtId'], $_POST['txtDescripcion']);
      if ($anwr === true) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      if (is_array($certificate)) {
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              $args['txtId'] = $data[0]['cod_tip_id'];
              $args['txtDescripcion'] = $data[0]['desc_tip_id'];
            }
          } else {
            $args['error'] = $data;
            viewClass::renderHTML('error.php', $args);
      ...
      $data['desc_tip_id'] = $_POST['txtDescripcion'];
      $rsp = modelClass::updateTid($_GET['id'], $data);
      if ($rsp === true) {
