...
class controllerClass {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $anwr = modelClass::newDepto($_POST['txtId'], $_POST['txtDepto']);
      if ($anwr === true) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      if (is_array($certificate)) {
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              $args['txtId'] = $data[0]['cod_depto'];
              $args['txtDepto'] = $data[0]['nom_depto'];
  ...
  public function delete() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' and isset($_GET['id']) and is_numeric($_GET['id'])) {
      ...
      $rsp = modelClass::delDepto($_GET['id']);
      if ($rsp === true) {
